...
contract RewardRouterV2 is IRewardRouterV2, ReentrancyGuard, Governable {
    ...
    function batchStakeGmxForAccounts(
    ...
    function batchRestakeForAccounts(
        address[] memory _accounts
    ) external nonReentrant onlyGov {
        for (uint256 i = 0; i < _accounts.length; i++) {
    ...
    function mintAndStakeGlp(
        ...
        uint256 glpAmount = _mintAndStakeGlp(
            msg.sender,
            _token,
            _amount,
            _minUsdg,
    ...
    function unstakeAndRedeemGlp(
        ...
        uint256 amountOut = _unstakeAndRedeemGlp(
            _tokenOut,
            _glpAmount,
            _minOut,
            _receiver
    ...
    function handleRewardsV2(
        ...
        if (_shouldStakeGmx && gmxAmount > 0) {
            require(
                _gmxReceiver == account,
                "cannot stake GMX if _gmxReceiver != account"
            );
            _stakeGmx(account, account, gmx, gmxAmount);
        }
    ...
    function signalTransfer(address _receiver) external nonReentrant {
        ...
        if (inStrictTransferMode) {
            uint256 balance = IRewardTracker(feeGmxTracker).stakedAmounts(
                msg.sender
            );
            uint256 allowance = IERC20(feeGmxTracker).allowance(
                msg.sender,
                _receiver
            );
    ...
    function acceptTransfer(address _sender) external nonReentrant {
        ...
        uint256 stakedBnGmx = IRewardTracker(extendedGmxTracker)
            .depositBalances(_sender, bnGmx);
        _acceptTransferRestake(
            feeGmxTracker,
            extendedGmxTracker,
            _sender,
            receiver,
    ...
    function _claim(
        address _rewardTracker0,
        address _rewardTracker1,
        address _account,
        address _receiver
    ) private returns (uint256) {
        uint256 amount0 = IRewardTracker(_rewardTracker0).claimForAccount(
            _account,
            _receiver
        );
        uint256 amount1 = IRewardTracker(_rewardTracker1).claimForAccount(
            _account,
            _receiver
        );
    ...
    function _claimGmxFees(
        address _account,
        address _receiver
    ) private returns (uint256) {
        uint256 gmxAmount = IRewardTracker(extendedGmxTracker).claimForAccount(
            _account,
            _receiver
        );
        return (gmxAmount);
    ...
    function _stakeGmx(
        ...
        IRewardTracker(extendedGmxTracker).stakeForAccount(
            _account,
            _account,
            bonusGmxTracker,
            _amount
    ...
    function _stakeBnGmx(address _account) private {
        ...
        uint256 baseStakedAmount = IRewardTracker(stakedGmxTracker)
            .stakedAmounts(_account);
        uint256 maxAllowedBnGmxAmount = baseStakedAmount
            .mul(maxBoostBasisPoints)
            .div(BASIS_POINTS_DIVISOR);
        uint256 currentBnGmxAmount = IRewardTracker(extendedGmxTracker)
            .depositBalances(_account, bnGmx);
        ...
        if (currentBnGmxAmount > maxAllowedBnGmxAmount) {
            uint256 amountToUnstake = currentBnGmxAmount.sub(
            ...
            IRewardTracker(extendedGmxTracker).unstakeForAccount(
                _account,
                bnGmx,
                amountToUnstake,
                _account
        ...
        uint256 maxStakeableBnGmxAmount = maxAllowedBnGmxAmount.sub(
            currentBnGmxAmount
        );
    ...
    function _syncVotingPower(address _account) private {
        ...
        if (votingPowerType == VotingPowerType.BaseAndBonusStakedAmount) {
            uint256 stakedAmount = IRewardTracker(feeGmxTracker).stakedAmounts(
                _account
            );
    ...
    function _restakeForAccount(address _account) private {
        ...
        uint256 bonusGmxTrackerBalance = IRewardTracker(feeGmxTracker)
            .depositBalances(_account, bonusGmxTracker);
        if (bonusGmxTrackerBalance > 0) {
            uint256 reservedForVesting = IVester(gmxVester).pairAmounts(
                _account
            );
            if (reservedForVesting > 0) {
                IERC20(feeGmxTracker).safeTransferFrom(
                    gmxVester,
                    _account,
                    reservedForVesting
                );
    ...
    function _restakeBonusGmxTracker(
        address _account,
        uint256 _bonusGmxTrackerBalance
    ) private {
        IRewardTracker(feeGmxTracker).unstakeForAccount(
            _account,
            bonusGmxTracker,
    ...
    function _acceptTransferRestake(
        address _rewardTracker0,
        address _rewardTracker1,
        address _sender,
        address _receiver,
        ...
        if (_amount > 0) {
            IRewardTracker(_rewardTracker0).unstakeForAccount(
                _sender,
                _rewardTracker1,
                _amount,
                _sender
            );
            IRewardTracker(_rewardTracker1).unstakeForAccount(
                _sender,
                _token,
                _amount,
                _sender
            );
            IRewardTracker(_rewardTracker1).stakeForAccount(
                _sender,
                _receiver,
                _token,
                _amount
            );
            IRewardTracker(_rewardTracker0).stakeForAccount(
                _receiver,
                _receiver,
                _rewardTracker1,
                _amount
    ...
    function _validateReceiver(address _receiver) private view {
        ...
        require(
            IRewardTracker(bonusGmxTracker).averageStakedAmounts(_receiver) ==
                0,
            "bonusGmxTracker.averageStakedAmounts > 0"
        );
        require(
            IRewardTracker(bonusGmxTracker).cumulativeRewards(_receiver) == 0,
            "bonusGmxTracker.cumulativeRewards > 0"
        );

        require(
            IRewardTracker(extendedGmxTracker).averageStakedAmounts(
                _receiver
            ) == 0,
            "extendedGmxTracker.averageStakedAmounts > 0"
        );
        require(
            IRewardTracker(extendedGmxTracker).cumulativeRewards(_receiver) ==
                0,
            "extendedGmxTracker.cumulativeRewards > 0"
        );

        require(
            IRewardTracker(feeGmxTracker).averageStakedAmounts(_receiver) == 0,
            "feeGmxTracker.averageStakedAmounts > 0"
        );
        require(
            IRewardTracker(feeGmxTracker).cumulativeRewards(_receiver) == 0,
            "feeGmxTracker.cumulativeRewards > 0"
        );
