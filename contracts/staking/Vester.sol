...
contract Vester is IVester, IERC20, ReentrancyGuard, Governable {
    ...
    function transferStakeValues(
        ...
        uint256 cumulativeReward = hasRewardTracker()
            ? IRewardTracker(rewardTracker).cumulativeRewards(_sender).sub(
                cumulativeRewardDeductions[_sender]
            )
            : 0;

        transferredCumulativeRewards[_receiver] = transferredCumulativeReward
            .add(cumulativeReward);
        cumulativeRewardDeductions[_sender] = cumulativeRewardDeductions[
            _sender
        ].add(cumulativeReward);
    ...
    function getMaxVestableAmount(
        ...
        uint256 maxVestableAmount = transferredCumulativeReward.add(
            bonusReward
        );
